...
const MASTER_FILE_COPY = '../dist/ffxiv-fashion-report-v2/master.json';
const INDENT_SIZE = 4;
const SLOT_TYPES = [
    'Head',
    'Body',
...
fs.copyFileSync(MASTER_FILE, MASTER_FILE_COPY);
checkMissingSlots();
...
function checkMissingSlots() {
    ...
    newReport.slots.forEach(reportSlot => {
        ...
        for (let index = 0; index < masterData.slots.length; index++) {
            ...
            foundSlot = slot.type === reportSlot.type && slot.hint.toLowerCase() === reportSlot.hint.toLowerCase();
